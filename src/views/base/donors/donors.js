...
const Donors = () => {
  ...
  const handleToggleStatus = async (id) => {
    ...
    try {
      // Toggle user status
      await axiosInstance.put(`${backendUrl}/apis/togglestatus/${id}`)
  ...
  const handleEdit = async (id) => {
    ...
    try {
      const response = await axiosInstance.get(`${backendUrl}/apis/donor/${id}`)
  ...
  const handleDelete = async (id) => {
    const userConfirmed = window.confirm('Are you sure you want to delete this donor?')
    ...
    try {
      ...
      if (editMode) {
        ...
        response = await axiosInstance.put(
          `${backendUrl}/apis/editdonor/${newDonors.id}`,
          formData,
          {
            headers: {
              'Content-Type': 'multipart/form-data',
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        console.log('Updated Donor:', updatedDonor) // Log updateAds to ensure it’s constructed correctly
            ...
            <div className="d-flex align-items-center">
              ...
              {!showForm && (
                <>
                  ...
                  <CButton color="primary" onClick={handleAddDonor}>
                    Add Donor
                  </CButton>
                </>
              )}
              ...
              <CForm className="row g-3 mb-4" onSubmit={handleFormSubmit}>
                ...
                <CCol md="6">
                  ...
                  {editMode && newDonors.photo && (
                    <div className="mb-3">
                      <img
                        src={`http://157.173.221.111:8080/communityapp.com/backend/${newDonors.photo}`}
                        alt="User Photo"
                        ...
                        <CTableRow>
                          ...
                          <CTableDataCell>
                            {donors.photo ? (
                              <img
                                src={
                                  `http://157.173.221.111:8080/communityapp.com/backend/${donors.photo}` ||
                                  ''
                                }
                                alt={donors.photo}
                                style={{
                                    ...
                                    <CCol md="12">
                                      ...
                                      {donors.photo && (
                                        <>
                                          <div>
                                            <img
                                              src={`http://157.173.221.111:8080/communityapp.com/backend/${donors.photo}`}
                                              alt="Donors Photo"
                                              style={{
                                                maxWidth: '100px',
                                                maxHeight: '100px',
